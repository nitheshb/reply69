...
class AdminGroupCsvDownload extends StatefulWidget {
    AdminGroupCsvDownload({Key key,this.ownerEmail,this.ownerName, this.ownerPhoneNumber,
  this.chatId, this.approvedGroupDetails});
  final String chatId;
  final List approvedGroupDetails;
  ...
  @override
  _AdminGroupCsvDownloadState createState() => _AdminGroupCsvDownloadState();
}

class _AdminGroupCsvDownloadState extends State<AdminGroupCsvDownload> {
...
//  Future<void> downloadFile(path) async {
//     Dio dio = Dio();
...
//     }
  getCsv() async {
    ...
    var cloud = await widget.approvedGroupDetails;
    ...
    if (cloud != null) {
      for (int i = 0; i < widget.approvedGroupDetails.length; i++) {
        List<dynamic> row = List<dynamic>();
        row.add(cloud[i]["userId"]);
        row.add(cloud[i]["membershipDuration"]);    
        row.add(cloud[i]["joinedId"]);
        row.add(cloud[i]["expiresOn"]);
  ...
  sendMailAndAttachment() async {
    final Email email = Email(
      ...
      recipients: [AdminGroupCsvDownload._emailController.text],
      isHTML: true,
            ...
            child: Column(
              ...
              children: <Widget>[
                Padding(
                  ...
                  child: TextFormField(
                    controller: AdminGroupCsvDownload._emailController,
